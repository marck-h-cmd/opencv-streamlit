    ...
    if st.session_state.get('tracking_active', False):
        ...
        class ObjectTracker:
            ...
            def start_tracking(self):
                if self.cap is None:
                    st.error("La cámara no está disponible")
                    return
                ...
                while self.cap.isOpened() and not stop_button:
                    ...
                    if self.frame is None:
                        st.error("Frame vacío capturado")
                        break
                        ...
                        if self.tracking_state == 1:
                            ...
                            if self.track_window and self.track_window[2] > 0 and self.track_window[3] > 0:
                                try:
                                    ...
                                    st.warning(f"Error en seguimiento: {e}")
                                    pass
        ...
        try:
            ...
            st.error(f"Error inicializando el rastreador: {e}")
            st.info("""
            ⚠️ **Nota:** La funcionalidad de seguimiento en tiempo real requiere acceso a la cámara web. 
            En entornos como Streamlit Cloud, esta funcionalidad puede no estar disponible.
            
            **Solución:**
            - Ejecuta la aplicación localmente para usar la cámara web
            - Asegúrate de que la cámara esté conectada y disponible
            - Otorga los permisos necesarios para acceder a la cámara
            """)
    
    else:
        st.info("👆 Presiona 'Iniciar Rastreador' para comenzar el seguimiento de objetos")
